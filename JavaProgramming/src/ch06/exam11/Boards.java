		...
		while (true) {
			...
			if (choice.equals("1")) {	// 목록보기
				...
				for (Board board : boards) {
					if (board != null) {
						System.out.println(board.no + "\t\t" + board.title + "\t" + board.writer + "\t\t" + board.count);
					}
				}
...
//						int noticeNum = Integer.parseInt(notice[0]);
						if(detailNum == board.no){
							board.count++;
							
							System.out.println("********************************************************");
							System.out.println("번호\t\t제목\t\t내용\t\t글쓴이\t\t조회수");
							System.out.println("********************************************************");
							System.out.println(board.no + "\t\t" + board.title + "\t" + board.content + "\t\t" + board.writer + "\t\t" + board.count);
						}
					}
				}
				...
				if (boards[modifyNum] != null) {
					...
					boards[modifyNum].title = title;
					boards[modifyNum].content = text;
				}
